...
class Game:
    ...
    def run(self):
        ...
        Player.initPieces(self.player1, self.tileOffset, self.tileSize, self.player1.color) 
    ...
    def draw_board(self):
        ...
        for row in range (20):
            for col in range(20):
                pg.draw.rect(board, self.tileColor, pg.Rect(self.borderSize + (col * self.tileOffset), self.borderSize + (row * self.tileOffset), self.tileSize, self.tileSize))
        return board
    ...
    def commitToBoard(self, player, piece):
        ...
        rowTileStart = int((piece.y - self.boardStartY - self.borderSize) / self.tileOffset)
        colTileStart = int((piece.x - self.boardStartX - self.borderSize) / self.tileOffset)
        ...
        for row in np.arange(self.tileOffset / 2, piece.height, self.tileOffset):
            for col in np.arange(self.tileOffset / 2, piece.width, self.tileOffset):
                if piece.image.get_at((row, col)) == piece.color:
